/*******************************************************************************
* This is the main program for Example 13 using PTK40A PIC Training Board.
*
...
int main(void)
{
	...
	// PTK40A come with an digital input channel
	// It can use to read digital signal from sensor 
	// This example will use digital sensor to detect obstacle, IR01A
	// This is the digital IR sensor which detect item when it is in range of it detection area 
	// So as a brief explaination when the IR01A detected obstacle, it will give a LOW (0V) at S (signal) pin
	// When there is no obstacle in range, it will output HIGH (5V), as simple as that
	while(1) 	// create an infinite loop
	{	
		if(DIGITAL_IN == 0)				
		{
			BUZZER = 1;	//if sensor detected obstacle, the buzzer will sound		
		}
		else
		{
			BUZZER = 0;	
