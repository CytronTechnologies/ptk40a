/*******************************************************************************
* This is the main program for Example 27 using PTK40A PIC Training Board.
*
...
int main(void)
{
	...
	// DIR to PWM signal, PWM to active high signal
	// Please refer to PTK40A schematic for the connection 
	// Please move JP10 to PWM, please check the connection according the examples guide
	...
	while(1) 	// create an infinite loop
	{
		...
		{
			speed = 10;
			pwm_set_duty_cycle(speed);	// change the direction and speed
			M1 = 1;
			lcd_goto(0x44);
			lcd_bcd(4, speed);
		}			
		...
		{
			speed = 1000;
			pwm_set_duty_cycle(speed);	// change directian and speed
			M1 = 1;	
			lcd_goto(0x44);
			lcd_bcd(4, speed);
		}
		...
		{
			speed = 512;
			pwm_set_duty_cycle(speed);	// stop the motor
			lcd_goto(0x44);
			lcd_bcd(4, speed);
		}
